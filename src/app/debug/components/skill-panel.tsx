...
name: happyimage-search
description: 当用户需要联网搜索、查询最新信息、核实事实或需要来源链接时，调用本地 HappyImage 搜索接口。
---
...
name: happyimage-search
description: Use when current web search is needed through this HappyImage server. Call the configured HTTP search endpoint with a prompt and return the answer with source URLs.
---

# HappyImage Search

Use this skill when the user asks for current web search, online lookup, recent information, or source-backed answers. It calls the local HappyImage search endpoint and returns an answer with source links.
...
2. Skill name: happyimage-search
