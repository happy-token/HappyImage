...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  ...
  const imageStorageMode: ImageStorageMode = imageStorage.enabled && imageStorage.mode === "both"
    ...
    : imageStorage.enabled && imageStorage.mode === "webdav"
      ? "webdav"
      : "local";
  const oidc = typeof config.oidc === "object" && config.oidc
  ...
  return {
    ...
    image_storage: {
      ...
      webdav_root_path: String(imageStorage.webdav_root_path || "happytoken/images"),
    ...
    try {
      const data = await updateSettingsConfig({
        ...
        image_storage: {
          ...
          webdav_root_path: String(config.image_storage?.webdav_root_path || "happytoken/images").trim(),
        ...
        oidc: {
          ...
          allowed_email_domains: String(config.oidc?.allowed_email_domains || "").trim(),
        },
      });
      set({ config: normalizeConfig(data.config) });
      toast.success("配置已保存");
