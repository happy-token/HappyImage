...
export function GalleryBrowser({ embedded = false, onUsePrompt }: GalleryBrowserProps = {}) {
  ...
  const [watermarkStatus, setWatermarkStatus] = useState("all");
  ...
  const watermarkStatuses = useMemo(
    () => Object.entries(facets?.watermark_statuses || {}).sort((a, b) => a[0].localeCompare(b[0])),
    [facets],
  );
  ...
  const loadItems = useCallback(
    async (nextOffset = 0, append = false) => {
      ...
      try {
        const data = await fetchSeedGallery({
          ...
          watermark_status: watermarkStatus === "all" ? "" : watermarkStatus,
          limit: 48,
          offset: nextOffset,
        });
    ...
    [category, query, watermarkStatus],
  );
    ...
    >
      <div className="flex flex-col gap-4 rounded-lg border border-stone-200/80 bg-white/74 p-4 shadow-sm dark:border-white/10 dark:bg-stone-950/62 sm:p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl space-y-2">
            ...
            <p className="text-sm leading-6 text-stone-600 dark:text-stone-300">
              已接入开源提示词素材库。图片和提示词可用于灵感与复刻起点，发布到正式商业图库前仍需要水印、商标和肖像复核。
            </p>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center sm:min-w-[360px]">
            ...
            <div className="rounded-md bg-stone-100/80 px-3 py-2 dark:bg-white/8">
              <div className="text-lg font-semibold text-stone-950 dark:text-stone-50">
                {facets?.watermark_statuses?.suspected_from_prompt ?? 0}
              </div>
              <div className="text-xs text-stone-500">疑似水印</div>
            </div>
          </div>
        </div>

        <div className="grid gap-2 lg:grid-cols-[minmax(0,1fr)_210px_210px]">
          ...
          <Select value={watermarkStatus} onValueChange={setWatermarkStatus}>
            <SelectTrigger className="h-11 rounded-2xl bg-white/90">
              <SelectValue placeholder="水印状态" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">全部水印状态</SelectItem>
              {watermarkStatuses.map(([value, count]) => (
                <SelectItem key={value} value={value}>
                  {formatWatermarkStatus(value)} ({count})
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-stone-500 dark:text-stone-400">
          ...
          <span>当前展示 {items.length} / {total} 条；疑似水印、品牌或真实人物素材请先复核再对外发布。</span>
        </div>
      </div>
