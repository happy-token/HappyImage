...
默认组合部署不需要 `.env`。只有要覆盖存储、数据库、端口或镜像名时，才在工作区根目录创建 `.env`；这些变量只属于基础设施层。`deploy/hs/docker-compose.yml` 不读取 `api/.env`，只显式传递基础设施环境变量：
...
注意：首次启动前必须创建 `data/` 父目录并把 `api/config.example.json` 复制为 `data/config.json`。不要让 Docker 把 `data/config.json` 自动创建成目录。
