...
每一条帖子发布前都能看到**真实平台 mockup 预览**。图片、标题、正文、话题标签在各平台上的呈现效果一目了然——包含实时字符统计、图片网格布局和平台规则校验。
...
<table>
<tr><th>风格</th><th>配色</th><th>排版</th></tr>
...
一键生成平台适配的推广文案。每个平台有独立的格式模板——小红书话题标签在末尾、微博 <code>#双#号#</code> 内联、微信无标签、X 内联 <code>#tag</code> 不超 280 字符。Chrome 自动化填入草稿，你只需审视确认后手动点击发布。

</td>
<td width="50%">

<h3>CLI · Web UI · GUI 三种接口</h3>
...
或直接告诉 Agent：*"请从 github.com/happy-token/HappyImage 安装 Skills"*
...
├── packages/
│   ├── core/        # @happytokenai/happyimage-core —— 共享运行时和 AI 流水线
...
HappyImage 基于 [baoyu-skills](https://github.com/JimLiu/baoyu-skills) 构建，内核的 AI 流水线、技能定义和多平台发布引擎均源自 baoyu-skills 生态。感谢 [JimLiu](https://github.com/JimLiu) 及社区奠定的基础。

由 [happy-token](https://github.com/happy-token) 及社区贡献者维护。
