...
当前仓库采用正式 monorepo 结构：

- `skills/`：独立 baoyu skills。
- `packages/core/`：HappyImage 共享运行时。
- `packages/web-ui/`：Hono API + React Web UI。
- `packages/cli/`：`happyimage` CLI。
- `packages/desktop/`：Electron 桌面端封装。
- `screenshots/`：文档和 Web UI 共用的风格预览资源。

完整目录约定见 [docs/project-structure.md](./docs/project-structure.md)。
