...
COMPOSE_ENV_FILE = ROOT_DIR / "tmp" / "docker-compose.env"
...
def write_compose_env(values: dict[str, str], dev_env: dict[str, str], output: Path, mode: str) -> dict[str, str]:
    output.parent.mkdir(parents=True, exist_ok=True)
    next_values: dict[str, str] = {}
    for key in (
        "HAPPYTOKEN_API_IMAGE",
        "HAPPYTOKEN_WEB_IMAGE",
        "HAPPYTOKEN_API_PORT",
        "HAPPYTOKEN_WEB_PORT",
        "STORAGE_BACKEND",
        "DATABASE_URL",
    ):
        value = os.environ.get(key) or dev_env.get(key, "")
        if value:
            next_values[key] = value

    if "DATABASE_URL" in next_values:
        next_values["DATABASE_URL"] = rewrite_dsn_for_mode(next_values["DATABASE_URL"], mode)

    lines = [
        "# Generated by scripts/generate-local-config.py.",
        f"# Mode: {mode}",
        "",
    ]
    for key in sorted(next_values):
        lines.append(f"{key}={next_values[key]}")
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return next_values
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optionally write a Docker Compose env file with mode-adjusted DATABASE_URL.",
    )
    ...
    if args.compose_env_output:
        compose_values = write_compose_env(values, dev_env, args.compose_env_output, args.mode)

    print("Read env:", args.env_file)
    print("Generated:", args.output)
    if args.compose_env_output:
        print("Generated compose env:", args.compose_env_output)
        if "DATABASE_URL" in compose_values:
            print("Compose DATABASE_URL:", redact_url(compose_values["DATABASE_URL"]))
