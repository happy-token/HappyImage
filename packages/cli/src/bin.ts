...
import { readSettings, PROJECT_ROOT, resolveSkillsRoot, resolveConfigRoot } from '@happytokenai/happyimage-core'

// Resolve @happytokenai/happyimage-web root directory
const webUiEntryFile = fileURLToPath(import.meta.resolve('@happytokenai/happyimage-web'))
...
async function web() {
  ...
  const serverModule = await import('@happytokenai/happyimage-web')
...
function config() {
  ...
  console.log(`  config .env (primary)`)
  ...
  if (!skillsRoot.ready) {
    console.log(`\nSkills not ready. Missing core skills: ${skillsRoot.missing.join(', ')}`)
  }
}
